...
export const pickerUtils = (function () {
	...
	let dateAllTimeArrayToString = (pickedValue)=>{
		let dateString = pickedValue[0] + "-" + fillZeroForNum(pickedValue[1]) + "-" + fillZeroForNum(pickedValue[2]);
		let hour24 = pickedValue[4];
		if(pickedValue[3] == '下午'){
			hour24 = parseInt(pickedValue[4]) + 12;
		}
		return moment(dateString + ' ' + fillZeroForNum(hour24) + ':' + fillZeroForNum(pickedValue[5])).format('YYYY-MM-DD HH:mm:ss');
	}
	...
	return {
		...
		dateAllTimeArrayToString: dateAllTimeArrayToString,
